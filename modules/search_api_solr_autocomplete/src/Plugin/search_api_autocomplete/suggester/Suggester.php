  ...
  public function getSuggesterSuggestions(SolrBackendInterface $backend, QueryInterface $query, SearchInterface $search, $incomplete_key, $user_input, array $options = []) {
    ...
    if ($solarium_query = $this->getAutocompleteQuery($backend,$incomplete_key, $user_input)) {
      try {
        ...
        \Drupal::moduleHandler()->alterDeprecated('hook_search_api_solr_suggester_autocomplete_query_alter is deprecated will be removed in Search API Solr 4.3.0. Handle the PreSpellcheckQueryEvent instead.', 'earch_api_solr_suggester_autocomplete_query_alter', $solarium_query, $query);
